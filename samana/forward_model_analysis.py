...
def forward_model(output_path,
                  ...
                  tolerance,
                  magnification_class,
                  dark_matter_model_class,
                  readout_macromodel_samples=True,
                  verbose=False,
                  random_seed_init=None,
                  readout_steps=2,
                  write_sampling_rate=True,
                  n_pso_particles=10,
                  n_pso_iterations=50,
                  num_threads=1,
                  astrometric_uncertainty=True,
                  ...
                  save_for_analysis_tolerance=0.0,
                  save_for_analysis_max_magnification=None,
                  save_for_analysis_min_mu_discrepancy=None,
                  lens_id=None,
    ...
    :param kwargs_sample_macro_fixed: a dictionary specifying priors on macromodel parameters, same syntax as kwargs_sample_realization
    ...
    :param magnification_class: an instance of a magnification class that computes image fluxes for a given lens model
    :param dark_matter_model_class: an instance of a dark matter model class that creates a realization
    ...
    :param n_pso_iterations: number of PSO iterations to use when modeling imaging data
    :param num_threads: number of threads for parallelization; WARNING, this appears to mess with the reproducibility/
    ...
    :param image_data_grid_resolution_rescale: rescales the resolution of the image grid for imaging data calculations
    ...
    :param scipy_minimize_method: string that specifies the minimize method used by scipy when solving for point source
    positions without imaging data
    ...
    :param fr_logL_source_reconstruction: the same functionality as tolerance_source_reconstruction, but triggers
    ...
    :param save_for_analysis: bool; after each realization is computed, recompute the image magnifications with the
    complementary magnification method (exact ray tracing if the run uses near/far splitting, and vice versa), display
    an inspection figure showing both, print a numerical comparison, and ask the user whether to save the model to disk
    for later analysis. Saved models are written as gzipped JSON and can be re-loaded with
    ...
    :param save_for_analysis_tolerance: only prompt when the two magnification methods disagree by more than this
    fraction. The comparison is made on both the per-image magnifications and the flux ratios, and the larger of the two
    decides. The default of 0.0 prompts for every realization that passes the pre-filters below
    :param save_for_analysis_max_magnification: skip realizations in which any image is brighter than this, which are
    usually images sitting on the critical curve rather than interesting failures of the approximation. None disables
    the cut
    :param save_for_analysis_min_mu_discrepancy: skip realizations in which the largest point-source magnification
    discrepancy reported by the near/far routines is below this value. This is a free pre-filter, since the near/far
    calculation already computes it, and avoids paying for the exact ray tracing on models that are obviously fine.
    ...
    while True:
        ...
        param_names_macro_fixed, _, _, _, source_plane_image_solution = forward_model_single_iteration(
            data_class,
            model,
            dark_matter_model_class,
            kwargs_sample_realization,
            ...
            kwargs_sample_source,
            kwargs_sample_macro_fixed,
            image_data_grid_resolution_rescale=image_data_grid_resolution_rescale,
            verbose=verbose,
            seed=random_seed,
            n_pso_particles=n_pso_particles,
            n_pso_iterations=n_pso_iterations,
            ...
            fitting_kwargs_list=fitting_sequence_kwargs,
            test_mode=test_mode,
            use_decoupled_multiplane_approximation=use_decoupled_multiplane_approximation,
            return_realization=return_realization,
            run_initial_PSO=run_initial_PSO,
            minimize_method=scipy_minimize_method,
            ...
            save_for_analysis_tolerance=save_for_analysis_tolerance,
            save_for_analysis_max_magnification=save_for_analysis_max_magnification,
            save_for_analysis_min_mu_discrepancy=save_for_analysis_min_mu_discrepancy,
            lens_id=lens_id,
...
def forward_model_single_iteration(data_class,
                                   model,
                                   dark_matter_model_class,
                                   kwargs_sample_realization,
                                   magnification_class,
    ...
    :param data_class:
    :param model:
    :param dark_matter_model_class:
    :param kwargs_sample_realization:
    :param magnification_class:
    ...
    :param save_for_analysis: bool; recompute the magnifications with the complementary method, display an inspection
    figure showing both, and prompt the user to save this lens model to disk for later analysis
    :param save_for_analysis_path: directory in which to write the saved model
    :param save_for_analysis_tolerance: only prompt when the two magnification methods disagree by more than this fraction
    :param save_for_analysis_max_magnification: skip realizations with an image brighter than this
    :param save_for_analysis_min_mu_discrepancy: skip realizations whose point-source discrepancy is below this
    ...
    # cheap pre-filters, evaluated before anything expensive happens. The point-source discrepancy is
    # already computed by the near/far routines, so it costs nothing to check
    consider_saving = save_for_analysis
    if consider_saving and save_for_analysis_max_magnification is not None:
        consider_saving = np.max(magnifications) < save_for_analysis_max_magnification
    if consider_saving and save_for_analysis_min_mu_discrepancy is not None:
        mu_discrepancy_run = getattr(magnification_class, 'mu_discrepancy', 0.0) or 0.0
        consider_saving = mu_discrepancy_run > save_for_analysis_min_mu_discrepancy
        if verbose and not consider_saving:
            print('skipping the magnification method comparison: largest point-source discrepancy '
                  + str(np.round(mu_discrepancy_run, 4)) + ' is below the threshold '
                  + str(save_for_analysis_min_mu_discrepancy))

    if consider_saving:
        # assemble a lightweight specification of this lens model, recompute the image magnifications
        # with the other magnification method, show both, and ask the user whether to write it to
        # disk. The saved file can be re-loaded and inspected with
        # samana.model_storage.LensSimulationAnalysis
        from samana.model_storage import (lens_model_spec_from_forward_model,
                                          prompt_save_lens_model, LensSimulationAnalysis)
        samples_dict = {}
        ...
        # recompute the same images with the complementary magnification method. The surfaces from
        # the run itself are passed straight through, so only one extra calculation is done
        simulation = LensSimulationAnalysis(spec)
        if verbose:
            print('recomputing the image magnifications with the other magnification method...')
        t_compare = time()
        comparison = simulation.compare_to_exact_ray_tracing(
            magnifications=magnifications,
            surfaces=images,
            tolerance=save_for_analysis_tolerance,
            verbose=verbose)
        if verbose:
            print('done in ' + str(np.round(time() - t_compare, 1)) + ' seconds; largest '
        ...
        if comparison['flagged']:
            ...
            print('the two magnification methods agree to within '
                  + str(np.round(100 * save_for_analysis_tolerance, 2)) + '%, not prompting')
